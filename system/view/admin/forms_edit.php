...
<tr bgcolor="#f9f9f9"><td <?=$td_style?> width="110"><?=$this->p_lang['model'].$this->p_lang['name']?></td><td  <?=$td_style?>><input type="text" name="name" id="name" <?=$input_style?> value="<?=$rs[0]['name']?>">
 <?
 if($type == 1){
 ?>&nbsp;&nbsp;
<?=$this->p_lang['login'].$this->p_lang['type']?>:
  <select name="login" id="login">
    <option value="0" <?=empty($rs[0]['login']) ? 'selected' : '' ?>>否</option>
    <option value="1"  <?=($rs[0]['login'] == 1) ? 'selected' : '' ?>>是</option>
  </select>
  <?
 }
  ?></td></tr></table>
<table class="table2" width="100%" border="0" cellspacing="0" id="tab">
<?
$field = empty($rs[0]['field']) ? array() : @unserialize($rs[0]['field']);
$i = 0;
foreach($field as $key => $val){
	?>
    <tr id="<?=$i?>"><td <?=$td_style?>><?=$this->p_lang['explanation']?></td><td <?=$td_style?>><input name='field_info[<?=$i?>]' type='text' id='field_info[<?=$i?>]' value='<?=$val['field_info']?>' class='kuangy'  /> <font color='#ff0000'>*</font> </td><td <?=$td_style?>><?=$this->p_lang['name']?></td><td <?=$td_style?>><input name='field_name[<?=$i?>]' type='text' value='<?=$val['field_name']?>' class='kuangy' /> <font color='#ff0000'>*</font></td><td <?=$td_style?>><?=$this->p_lang['type']?></td><td <?=$td_style?>>
    ...
    <option value='6' <?=($val['field_type'] == 6) ? 'selected' : ''?>><?=$this->p_lang['more'],$this->p_lang['pic'].$this->p_lang['upload']?></option>
    </select>
    
    </td><td <?=$td_style?>><?=$this->p_lang['parameter']?></td><td <?=$td_style?>><input name='field_parameter[<?=$i?>]' type='text' value='<?=$val['field_parameter']?>' class='kuangy' /> </td><td <?=$td_style?>><input name='' type='button' value='<?=$this->p_lang['delete']?>' onClick='del(<?=$i?>)' style='border:1px #000000 solid;vertical-align:middle;height:25px'></td></tr>
    <?
	$i++;
}
?>
