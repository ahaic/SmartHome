...
.plus {
	background:transparent url(static/images/plugin.gif) no-repeat 16px 16px;
	background-position:2px 2px;
}
...
$editor = 'tools:\'Bold,Italic,Underline,Strikethrough,FontColor,BackColor,|,Removeformat,Align,List,|,Link,Unlink,Img,Flash,Media,Table,|,Fontface,FontSize,|,test4,test2,Source,\',upImgUrl:\''.url(array('admin', 'ajax_upload')).'\',upImgExt:\'jpg,jpeg,gif,png\'';
?>
<script>
var allPlugin={
...
{
	window.location.href="<?=url(array('user', 'news_edit', $id))?>&cid="+$('#cate').val()+"";	
}
</script>
</head>
...
<tr bgcolor="#f9f9f9"><td <?=$td_style?> width="110"><?=$this->p_lang['content']?></td><td  <?=$td_style?>><textarea name="ncontent" id="ncontent" style="width:520px; height:400px" onBlur="this.className='kuang'" class="xheditor {skin:'nostyle',plugins:allPlugin,loadCSS:'<style>pre{margin-left:2em;border-left:3px solid #CCC;padding:0 1em;}</style>',forcePtag:false,<?=$editor?>}"><?=htmlentities($rs[0]['ncontent'], ENT_QUOTES, 'UTF-8')?></textarea></td></tr>
...
if(!empty($cfield))
{
	foreach ($cfield as $key => $val)
	{
		switch($val['field_type']){
			case 1:
			echo '<tr bgcolor="#f9f9f9"><td '.$td_style.' width="110">'.$val['field_info'].'</td><td  '.$td_style.'><input type="input" '.$input_style.' name="nfield['.$val['field_name'].']" id="nfield['.$val['field_name'].']" value="'.$field_arr[$val['field_name']].'" ></td></tr>';
			break;	
			case 2:
			echo '<tr bgcolor="#f9f9f9"><td '.$td_style.' width="110">'.$val['field_info'].'</td><td  '.$td_style.'><textarea name="nfield['.$val['field_name'].']" id="nfield['.$val['field_name'].']" style="width:600px; height:400px" onBlur="this.className=\'kuang\'" class="xheditor {skin:\'nostyle\','.$editor.'}">'.$field_arr[$val['field_name']].'</textarea></td></tr>';
			break;
			case 3:
			$temp_arr = array();
			$temp = '';
			...
			foreach($temp_arr as $k => $v){
				$selected = ($v == $field_arr[$val['field_name']])? 'checked="checked"' : '';
				$temp .= '<input name="nfield['.$val['field_name'].']" type="radio" value="'.$v.'" '.$selected.'>'.$v.' ';	
			}
			...
			$temp_arr = explode('|', $val['field_parameter']);
			foreach($temp_arr as $k => $v){
				$selected = in_array($v, $field_arr[$val['field_name']]) ? 'checked="checked"' : '';
				$temp .= '<input name="nfield['.$val['field_name'].'][]" type="checkbox" value="'.$v.'" '.$selected.'>'.$v.' ';	
			}
			echo '<tr bgcolor="#f9f9f9"><td '.$td_style.' width="110">'.$val['field_info'].'</td><td  '.$td_style.'>'.$temp.'</td></tr>';
			break;
			case 5:
...
$(document).ready(function(){
		...
		new AjaxUpload(button_<?=$val['field_name']?>, {
            ...
            <?
			echo '<tr bgcolor="#f9f9f9"><td '.$td_style.' width="110">'.$val['field_info'].'</td><td  '.$td_style.'><input type="input" '.$input_style.' name="nfield['.$val['field_name'].']" id="nimg_'.$val['field_name'].'" value="'.$field_arr[$val['field_name']].'" ><input type="button" value="'.$this->p_lang['upload'].'" id="upload_input_'.$val['field_name'].'"></td></tr>';
			break;
			case 6:
			?>
